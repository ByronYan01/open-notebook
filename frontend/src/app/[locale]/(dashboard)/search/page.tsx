      ...
      <div className="p-4 md:p-6">
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'ask' | 'search')} className="w-full space-y-6">
          <div className="space-y-2">
            ...
            <TabsList aria-label={t('askOrSearchAria')} className="w-full max-w-xl">
              ...
              <CardContent className="space-y-4">
                ...
                {!hasEmbeddingModel ? (
                  <div className="flex items-center gap-2 p-3 text-sm text-amber-600 dark:text-amber-500 bg-amber-50 dark:bg-amber-950/20 rounded-md">
                    ...
                    <span>{tAsk('noEmbeddingModel')}</span>
                  </div>
                ) : (
                  <>
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2 text-xs flex-wrap">
                        <Badge variant="secondary">
                          {tAsk('modelStrategy')}: {resolveModelName(customModels?.strategy || modelDefaults?.default_chat_model)}
                        </Badge>
                        <Badge variant="secondary">
                          {tAsk('modelAnswer')}: {resolveModelName(customModels?.answer || modelDefaults?.default_chat_model)}
                        </Badge>
                        <Badge variant="secondary">
                          {tAsk('modelFinal')}: {resolveModelName(customModels?.finalAnswer || modelDefaults?.default_chat_model)}
                        </Badge>
                      </div>
                    </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">{tSearch('submitHint')}</p>
                </div>
                ...
                <div className="space-y-4">
                  {/* Search Type */}
                  <div className="space-y-2">
                    <Label>{tSearch('searchTypeLabel')}</Label>
                    {!hasEmbeddingModel && (
                      <div className="flex items-center gap-2 text-sm text-amber-600 dark:text-amber-500">
                        <AlertCircle className="h-4 w-4" />
                        <span>{tSearch('noEmbeddingModelWarning')}</span>
                      </div>
                    )}
                    <RadioGroup
                      value={searchType}
                  ...
                  {/* Search Locations */}
                  <div className="space-y-2">
                    <Label>{tSearch('searchInLabel')}</Label>
                    <div className="space-y-2">
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          id="sources"
                  ...
                  <div className="mt-6 space-y-3">
                    <div className="flex items-center justify-between">
                      <h3 className="text-sm font-medium">
                        {tSearch('resultsCount', { count: searchMutation.data.total_count })}
                      </h3>
                      <Badge variant="outline">{tSearch('searchTypeBadge', { type: searchMutation.data.search_type })}</Badge>
                    </div>

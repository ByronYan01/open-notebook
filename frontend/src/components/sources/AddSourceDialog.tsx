...
export function AddSourceDialog({
  ...
  // Dynamic schema with i18n validation messages
  ...
  // Dynamic wizard steps with i18n
  ...
  const onSubmit = async (data: CreateSourceFormData) => {
    try {
      ...
      if (isBatchMode) {
        // Batch submission
        setProcessingStatus({
          message: t("processing.batch", { count: itemCount }),
        });
        const results = await submitBatch(data);

        // Show summary toast
        if (results.failed === 0) {
          toast.success(t("toast.batchSuccess", { count: results.success }));
        } else if (results.success === 0) {
          toast.error(t("toast.batchFailed", { count: results.failed }));
        } else {
          toast.warning(
        ...
        // Single source submission
        setProcessingStatus({ message: t("processing.single") });
      ...
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className="sm:max-w-[500px]" showCloseButton={true}>
          <DialogHeader>
            <DialogTitle>
              {batchProgress
                ? t("processing.batchTitle")
                : t("processing.singleTitle")}
            </DialogTitle>
            <DialogDescription>
              {batchProgress
                ? t("processing.batchDescription", {
                    count: batchProgress.total,
                  })
                : t("processing.singleDescription")}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-muted-foreground">
                {processingStatus?.message || t("processing.default")}
              </span>
            </div>
              ...
              <>
                ...
                <div className="flex items-center justify-between text-sm">
                  <div className="flex items-center gap-4">
                    <span className="flex items-center gap-1.5 text-green-600">
                      ...
                      {batchProgress.completed} {t("processing.completed")}
                    </span>
                    {batchProgress.failed > 0 && (
                      <span className="flex items-center gap-1.5 text-destructive">
                        <XCircleIcon className="h-4 w-4" />
                        {batchProgress.failed} {t("processing.failed")}
                      </span>
                    )}
                  </div>
                ...
                {batchProgress.currentItem && (
                  <p className="text-xs text-muted-foreground truncate">
                    {t("processing.current")}: {batchProgress.currentItem}
                  </p>
                )}
              </>
            )}

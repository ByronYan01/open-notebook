...
export function StreamingResponse({
  ...
  const t = useTranslations('search.streamingResponse')
  ...
  const handleReferenceClick = (type: string, id: string) => {
    ...
    try {
      ...
      const typeLabel = type === 'source_insight' ? t('type.insight') : t(`type.${type}` as any)
      toast.error(t('referenceNotFound', { type: typeLabel }))
    }
  }
          ...
          <Card>
            ...
            <CollapsibleContent>
              <CardContent className="space-y-3 pt-0">
                <div>
                  <p className="text-sm text-muted-foreground mb-2">{t('strategy.reasoning')}:</p>
                  <p className="text-sm">{strategy.reasoning}</p>
                </div>
                {strategy.searches.length > 0 && (
                  <div>
                    <p className="text-sm text-muted-foreground mb-2">{t('strategy.searchTerms')}:</p>
            ...
            <CardHeader>
              <CollapsibleTrigger className="flex items-center justify-between w-full hover:opacity-80">
                <CardTitle className="text-base flex items-center gap-2">
                  ...
                  {t('individualAnswers', { count: answers.length })}
                </CardTitle>
